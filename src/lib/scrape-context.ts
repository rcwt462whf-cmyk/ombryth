// Destination-page scraper — pulls title, description and structured product data
// (JSON-LD → OG tags → microdata) out of an affiliate page so captions can ground
// their click-teases in real product names/prices/ratings/scarcity.
//
// Lives in /lib so BOTH the /api/scrape-context route (user-facing, session-guarded)
// and the /api/generate-pin Vynthr bridge (server-to-server) can call it directly,
// instead of the bridge making an unauthenticated HTTP hop to the route (which 401s
// and silently drops all destination context).

export type ScrapedProduct = { name: string; price?: string; rating?: string; reviewCount?: string; scarce?: boolean }
...
// fallbacks for pages with no JSON-LD are handled by the caller using the getMeta/getMicrodata helpers.
...
// Strip control chars and anything that looks like prompt injection before this content
// enters an AI system prompt.
...
/** Returns true if the hostname points at a private/internal/metadata address (SSRF guard). */
...
/**
 * Fetch a URL and extract title, description and structured product data.
 * Never throws — returns an `error` field on failure so callers can degrade gracefully.
 */
export async function scrapeUrl(url: string): Promise<ScrapeResult> {
  ...
  // roundups), then OG product tags, then legacy schema.org microdata.
